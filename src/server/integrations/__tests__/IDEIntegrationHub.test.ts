import { IDEIntegrationHub, getIDEHub } from '../IDEIntegrationHub';
import { CursorConnector } from '../connectors/CursorConnector';
import { VSCodeConnector } from '../connectors/VSCodeConnector';
import { BidirectionalSyncManager, getSyncManager } from '../sync/BidirectionalSyncManager';

describe('IDE Integration Hub - Multi-IDE Architecture', () => {
  let ideHub: IDEIntegrationHub;
  let syncManager: BidirectionalSyncManager;
  ...
  describe('IDE Connectors', () => {
    describe('Cursor Connector', () => {
      ...
      it('should handle connection lifecycle', async () => {
        ...
        jest.spyOn(connector, 'doConnect').mockResolvedValue();

        await expect(connector.connect()).resolves.not.toThrow();

        // Mock successful disconnection
        jest.spyOn(connector, 'doDisconnect').mockResolvedValue();
  ...
  describe('Integration Scenarios', () => {
    it('should handle multi-IDE context synchronization', async () => {
      ...
      jest.spyOn(cursorConnector, 'doConnect').mockResolvedValue();
      jest.spyOn(vscodeConnector, 'doConnect').mockResolvedValue();

      ideHub.registerAdapter(cursorConnector);
      ideHub.registerAdapter(vscodeConnector);

      // Mock context sync
      jest.spyOn(cursorConnector, 'doSendContext').mockResolvedValue();
      jest.spyOn(vscodeConnector, 'doSendContext').mockResolvedValue();
  ...
  describe('Performance and Scalability', () => {
    it('should handle multiple adapters efficiently', async () => {
      ...
      for (const adapter of adapters) {
        jest.spyOn(adapter, 'doConnect').mockResolvedValue();
        jest.spyOn(adapter, 'checkAvailability').mockResolvedValue(true);
      }

      // Register all adapters
      for (const adapter of adapters) ideHub.registerAdapter(adapter);

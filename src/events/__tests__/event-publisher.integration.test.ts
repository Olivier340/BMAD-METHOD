...
class MockSSEServer {
  ...
  private targetedEvents: { eventData: any; clientIds: string[] }[] = [];

  publishEvent(eventData: any): void {
    this.broadcastedEvents.push(eventData);
  }

  publishEventTo(eventData: any, clientIds: string[]): void {
  ...
  getBroadcastedEvents(): any[] {
    return this.broadcastedEvents;
  }

  getTargetedEvents(): { eventData: any; clientIds: string[] }[] {
...
describe('EventPublisher Integration', () => {
  ...
  beforeEach(() => {
    ...
    eventPublisher.setSSEServer(mockSSEServer as any);
  });
